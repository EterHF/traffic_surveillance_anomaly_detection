...
class RAFTFeatureExtractor:
    ...
    def extract_features(self, images_bgr: list[np.ndarray], channel_last: bool = False) -> np.ndarray:
        """
        Return dense optical flow for each adjacent frame pair.

        Output shape:
        - default (channel_last=False): [num_pairs, 2, H, W]
        - channel_last=True: [num_pairs, H, W, 2]
        where num_pairs = len(images_bgr) - 1 and channel 2 is (dx, dy).
        """
        if len(images_bgr) < 2:
            raise ValueError("At least two images are required to extract RAFT features")
        ...
        flows = np.stack(features, axis=0).astype(np.float32)
        if channel_last:
...
if __name__ == "__main__":
    ...
    feature = extractor.extract_features([dummy_img_1, dummy_img_2])
    feature_hwc = extractor.extract_features([dummy_img_1, dummy_img_2], channel_last=True)
    ...
    print("Extracted feature shape (num_pairs, 2, H, W):", feature.shape)
    print("Extracted feature shape (num_pairs, H, W, 2):", feature_hwc.shape)
