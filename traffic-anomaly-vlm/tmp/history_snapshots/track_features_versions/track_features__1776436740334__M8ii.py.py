...
def build_track_features(
    ...
    for seq in filtered_by_id.values():
        ...
        for i in range(1, len(seq)):
            vx_i, vy_i = _velocity_at(seq, i, speed_span, fit_degree=fit_degree)
        ...
        for i in range(2, len(seq)):
            ...
            pred = _predict_with_history(seq[:i], dt=dt2, lookback=pred_lookback, fit_degree=fit_degree)
            if pred is None:
                continue
            ...
            v1x, v1y = _velocity_at(seq, i - 1, speed_span, fit_degree=fit_degree)
            v2x, v2y = _velocity_at(seq, i, speed_span, fit_degree=fit_degree)
            ...
            speed_turn_hit = speed_change_ratio > 0.5
            dir_turn_hit = abs(math.sin(dir2 - dir1)) > 0.5
            if speed_turn_hit:
