...
class SceneFeatureExtractor:
    ...
    def compute(
        ...
        if self.cfg.use_lowres and images_bgr:
            ...
            lowres_eventness = _summarize_pair_change(lowres_pair, len(images_bgr))

        track_layout_eventness = 0.0
        if self.cfg.use_track_layout and window_tracks:
        ...
        clip_eventness = 0.0
        if clip_weight > 0.0 and images_bgr:
            clip_pair = self._get_clip_extractor().compute_pair_errors(images_bgr)["eventvad_combined_error"]
            clip_eventness = _summarize_pair_change(clip_pair, len(images_bgr))

        raft_eventness = 0.0
        if raft_weight > 0.0 and images_bgr and self.cfg.raft_root and self.cfg.raft_weights:
            raft_pair = self._get_raft_extractor().compute_pair_errors(images_bgr)["eventvad_combined_error"]
            raft_eventness = _summarize_pair_change(raft_pair, len(images_bgr))
