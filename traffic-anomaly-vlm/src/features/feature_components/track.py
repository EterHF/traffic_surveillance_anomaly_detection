...
def _is_static_full_window(seq: list[TrackObject], window_len: int) -> bool:
    ...
    return (d / scale) < 0.05 # FIXME


def _predict_next_center(
    ...
    if len(seq) < 2:
        return None
    seq = sorted(seq, key=lambda x: x.frame_id)
    max_hist = min(len(seq), max(3, int(history_len or len(seq))))
    ...
    return _predict_with_history(seq, dt=dt, lookback=max_hist, fit_degree=fit_degree)
...
def _velocity_at(seq: list[TrackObject], end_idx: int, span: int, fit_degree: int = 2) -> tuple[float, float]:
    start_idx = max(0, end_idx - span + 1)
    sub = seq[start_idx : end_idx + 1]
    fitted = _poly_velocity_and_position(sub, degree=fit_degree)
    if fitted is None:
        return 0.0, 0.0
    vx, vy, _, _ = fitted
    return vx, vy
...
def _predict_with_history(
    ...
    try:
        px = _fit_poly_1d(t, x, degree=fit_degree)
        py = _fit_poly_1d(t, y, degree=fit_degree)
    except Exception:
        return None
...
def build_track_features(
    ...
    fit_hist = int(history_len) if history_len is not None else int(window_len)
    fit_hist = max(3, min(max(3, window_len), fit_hist)) if window_len > 0 else 3
    fit_degree = max(1, int(fit_degree))

    speed_span = max(2, fit_hist)
    pred_lookback = max(3, fit_hist)
    ...
    for i in range(max(0, len(window_tracks) - 1)):
        ...
        for tid in disappeared:
            ...
            disappear_far_flags.append(1.0 if d_norm > 0.08 else 0.0) # FIXME
    ...
    for tid in current_ids:
        ...
        # Current-frame speed (at the last point).
        v2x, v2y = _velocity_at(seq, len(seq) - 1, speed_span, fit_degree=fit_degree)
        n2 = math.hypot(v2x, v2y)
        scale_cur = math.sqrt(max(seq[-1].area, 1.0))
        current_speeds.append(n2 / scale_cur)

        # Current-frame bbox/center prediction error: predict current point from previous history.
        if len(seq) >= 3:
            p1 = seq[-2]
            p2 = seq[-1]
            dt2 = max(1, int(p2.frame_id - p1.frame_id))
            pred = _predict_with_history(seq[:-1], dt=dt2, lookback=pred_lookback, fit_degree=fit_degree)
            if pred is not None:
                pred_cx, pred_cy = pred
                current_pred_center_errs.append(math.hypot(p2.cx - pred_cx, p2.cy - pred_cy) / scale_cur)
        ...
        if len(seq) >= 3:
            v1x, v1y = _velocity_at(seq, len(seq) - 2, speed_span, fit_degree=fit_degree)
            n1 = math.hypot(v1x, v1y)
            turn_candidates += 1
            speed_change_ratio = abs(n1 - n2) / max(1e-3, (n1 + n2) * 0.5)
            speed_turn_rates.append(speed_change_ratio)

            # Keep direction-change metric as an auxiliary current-frame signal.
            ...
            if speed_change_ratio > 0.5 or dir_change_ratio > 0.5:
                turn_active += 1
    ...
    for tid in current_ids:
        seq = sorted(filtered_by_id.get(tid, []), key=lambda x: x.frame_id)
        pred = _predict_next_center(seq, fit_degree=fit_degree, history_len=fit_hist)
        if pred is not None:
            pred_next[tid] = pred
    ...
    if len(pred_ids) < 2:
        min_next_dist = 1.0
    collision_warning = math.exp(-min_next_dist / 0.03) # FIXME
    ...
    return {
        ...
        "pred_center_sum_err": sum(current_pred_center_errs),
        "pred_iou_sum_err": sum(current_pred_iou_errs),

        "speed_turn_rates_max": speed_turn_rates_max,
        "speed_turn_rates_sum": sum(speed_turn_rates),

        "dir_turn_rates_max": dir_turn_rates_max,
        "dir_turn_rates_sum": sum(dir_turn_rates),
