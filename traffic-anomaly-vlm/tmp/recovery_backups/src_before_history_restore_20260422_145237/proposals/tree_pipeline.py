...
def refine_event_nodes_with_vlm(
	...
	for node in nodes:
		evidence = evidences.get(node.proposal_id)
		if evidence is None:
...
class TreePipeline:
	...
	def process(self, 
		...
		if self.tree_build_cfg.refine_coarse:
			evidences = [self.evidence_builder.build(
				proposal=node,
				images=images,
				frame_ids=frame_ids,
		...
		tree = build_event_tree_from_coarse_nodes(frame_ids, scores, refined_coarse, self.tree_build_cfg)
