...
def build_track_features(window_tracks: list[list[TrackObject]]) -> dict[str, float]:
    ...
    for seq in filtered_by_id.values():
        ...
        if len(seq) < 2:
            continue
        # Speed estimation using simple frame-to-frame displacement normalized by object size.
        ...
        # bboxes prediction using simple constant velocity model and error estimation.
        for i in range(2, len(seq)):
            ...
            # Check: speed magnitude sharp change and direction change
            # Turn rate estimation using cosine of angle between consecutive velocity vectors.
            ...
            turn_candidates += 1
            if abs(n1 - n2) / max(1e-6, n1) > 0.4:
                turn_active += 1
                speed_turn_rates.append(abs(n1 - n2) / max(1e-6, n1))
            if abs(math.sin(dir2 - dir1)) > 0.35:
                turn_active += 1
