...
def _predict_next_center(seq: list[TrackObject]) -> tuple[float, float] | None:
    ...
    vx, vy = _robust_velocity(seq[-max_hist:])
    ...
    p1 = seq[-1]
    return (p1.cx + vx * dt, p1.cy + vy * dt)
...
def _robust_velocity(seq: list[TrackObject]) -> tuple[float | None, float | None]:
    if len(seq) < 2:
        return None, None
    vxs: list[float] = []
    vys: list[float] = []
    for i in range(1, len(seq)):
        dt = max(1, int(seq[i].frame_id - seq[i - 1].frame_id))
        vxs.append((seq[i].cx - seq[i - 1].cx) / float(dt))
        vys.append((seq[i].cy - seq[i - 1].cy) / float(dt))
    vxs = sorted(vxs)
    vys = sorted(vys)
    mid = len(vxs) // 2
...
def _velocity_at(seq: list[TrackObject], end_idx: int, span: int) -> tuple[float, float]:
    ...
    vx, vy = _robust_velocity(sub)
    return float(vx or 0.0), float(vy or 0.0)
...
def _predict_with_history(history: list[TrackObject], dt: int, lookback: int) -> tuple[float, float] | None:
    ...
    max_hist = min(len(hist), max(2, lookback))
    vx, vy = _robust_velocity(hist[-max_hist:])
    if vx is None or vy is None:
        return None
    p = hist[-1]
    return (p.cx + vx * dt, p.cy + vy * dt)
...
def build_track_features(window_tracks: list[list[TrackObject]]) -> dict[str, float]:
    ...
    speed_span = max(2, min(6, window_len))
    pred_lookback = max(3, min(8, window_len))
